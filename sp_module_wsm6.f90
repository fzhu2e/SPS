...
!=================================================
SUBROUTINE mp_wsm6(its,ite,kts,kte,mid,wGrid)
IMPLICIT NONE
INTEGER, INTENT(IN) :: its, ite, kts, kte
TYPE(grid), INTENT(INOUT) :: wGrid
TYPE(mainvar), INTENT(OUT) :: mid
INTEGER :: i, k
!-------------------------------------------------
! INOUT
!----------
REAL(kd), DIMENSION(its:ite,kts:kte) :: t, q
REAL(kd), DIMENSION(its:ite,kts:kte,2) :: qci
REAL(kd), DIMENSION(its:ite,kts:kte,3) :: qrs
REAL(kd), DIMENSION(ims:ime) :: rain, rainncv, sr, snow, snowncv, graupel, graupelncv
!!-------------------------------------------------
! IN - but comment them !!
!----------
REAL(kd), DIMENSION(ims:ime,kms:kme) :: den, p, delz
!REAL(kd) :: delt, g, cpd, cpv, t0c, den0, rd, rv, ep1, ep2, qmin, &
            !XLS, XLV0, XLF0, cliq, clice, psat, denr
REAL(kd) :: delt
!!-------------------------------------------------
! LOCAL
!----------
REAL(kd), DIMENSION(its:ite,kts:kte,3) :: rh, qs, rslope, rslope2, rslope3, rslopeb, &
                                          qrs_tmp, falk, fall, work1
...
REAL(kd), DIMENSION(its:ite,kts:kte) :: Pigen, Pidep, Pcond, Prevp, Psevp, &
                                        Pgevp, Psdep, Pgdep, Praut, Psaut, &
                                        Pgaut, Piacr, Pracw, Praci, Pracs, &
                                        Psacw, Psaci, Psacr, Pgacw, Pgaci, &
                                        Pgacr, Pgacs, Paacw, Psmlt, Pgmlt, &
                                        Pseml, Pgeml
...
REAL(kd), DIMENSION(its:ite) :: tstepsnow, tstepgraup, mstep, numdt, flgcld

REAL(kd) :: cpmcal, xlcal, diffus,                                &
...
REAL(kd), DIMENSION( its:ite ) ::                             tvec1
REAL(kd)                       ::                              temp
!
!=================================================================
...
DO i = its, ite
	rain(i) = wGrid%rain(i)
	rainncv(i) = wGrid%rainncv(i)
	sr(i) = wGrid%sr(i)
	snow(i) = wGrid%snow(i)
	snowncv(i) = wGrid%snowncv(i)
	graupel(i) = wGrid%graupel(i)
	graupelncv(i) = wGrid%graupelncv(i)
END DO
p = p0*EXP(LOG(wGrid%pi)*(Cp/Rd))
den = p/Rd/t
DO k = kts, kte
	delz(:,k) = dk(k)
END DO
...
DO k = kts, kte
	DO i = its, ite
		mid%theta(i,k) = t(i,k)/wGrid%pi(i,k)
		mid%qv(i,k) = q(i,k)
		mid%qc(i,k) = qci(i,k,1)
		mid%qi(i,k) = qci(i,k,2)
		mid%qr(i,k) = qrs(i,k,1)
		mid%qs(i,k) = qrs(i,k,2)
		mid%qg(i,k) = qrs(i,k,3)
	END DO
END DO
DO i = its, ite
	wGrid%rain(i) = rain(i)
	wGrid%rainncv(i) = rainncv(i)
	wGrid%sr(i) = sr(i)
	wGrid%snow(i) = snow(i)
	wGrid%snowncv(i) = snowncv(i)
	wGrid%graupel(i) = graupel(i)
	wGrid%graupelncv(i) = graupelncv(i)
END DO
!=================================================
!CALL debug_SFSG
!=================================================
...
!.... constants which may not be tunable
   REAL(kd), INTENT(IN) :: den0,denr,dens,cl,cpv
