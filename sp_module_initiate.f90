...
! z_hat(i,k), z_hat_pi(i,k), z_hat_u(i,k), z_hat_vir(i,k)  =  zz(k) + b(k)*zs(i)
! PbPzhat(i,k), PbPzhat_pi(i,k), PbPzhat_u(i,k), PbPhat_vir(i,k)
! VertA_u(i), VertA_pi(i) = PzsPx(i)
! VertB_u(i,k), VertB_pi(i,k), VertB_u(i,k), VertB_vir(i,k)
! VertC_u(i,k), VertC_pi(i,k), VertC_u(i,k), VertC_vir(i,k)
!=================================================
!SUBROUTINE initiate_virertcoords
!IMPLICIT NONE
...
!END DO
!!-------------------------------------------------
!CALL debug_undef_all(z_hat,z_hat_pi,z_hat_u,z_hat_vir)
!CALL debug_undef_all(PbPzhat,PbPzhat_pi,PbPzhat_u,PbPzhat_vir)
!CALL debug_undef_all(VertB_w,VertB_pi,VertB_u,VertB_vir)
!CALL debug_undef_all(VertC_w,VertC_pi,VertC_u,VertC_vir)
...
!DO i = imin, imax
	!DO k = kmin, kmax
		!z_hat_vir(i,k) = zz(k) + zs(i)*b(k)
		!PbPzhat_vir(i,k) = - COSH((ztop - z_hat_vir(i,k))/sh)/SINH(ztop/sh)/sh
		!VertB_vir(i,k) = 1 + zs(i)*PbPzhat_vir(i,k)
		!VertC_vir(i,k) = b(k)*VertA_u(i)/VertB_vir(i,k)
	!END DO
!END DO
...
!!=================================================
!END SUBROUTINE initiate_virertcoords
!=================================================
